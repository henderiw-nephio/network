...
func (r *network) PopulateBridgeInterface(ctx context.Context, bdName string, vlanId uint16, ep invv1alpha1.Endpoint) error {
...
func (r *network) PopulateRoutedInterface(ctx context.Context, rtName string, vlanId uint16, ep invv1alpha1.Endpoint) error {
	nodeName := ep.Spec.NodeName
	if _, ok := r.devices[nodeName]; !ok {
		r.devices[nodeName] = new(ygotsrl.Device)
	}
...
func (r *network) PopulateIRBInterface(ctx context.Context, routed bool, bdName, rtName string, ep invv1alpha1.Endpoint) error {
	nodeName := ep.Spec.NodeName
	if _, ok := r.devices[nodeName]; !ok {
		r.devices[nodeName] = new(ygotsrl.Device)
	}
